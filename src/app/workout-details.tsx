  ...
  return (
    <SafeAreaView className="flex-1 bg-bg">
      <ScrollView className="flex-1">
        ...
        {/* Summary Card */}
        <View className="bg-surface rounded-xl p-4 mb-6 mx-4 mt-4">
          <View className="flex-row justify-between items-start mb-3">
            ...
            <View className={`px-3 py-1 rounded-full ${session.completed_at ? 'bg-green-500/20' : 'bg-blue-500/20'}`}>
              <Text className={`text-sm font-bold ${session.completed_at ? 'text-green-400' : 'text-blue-400'}`}>
        ...
        <View className="px-4 mb-8">
          <Text className="text-text text-xl font-bold mb-4">Exercises</Text>
          ...
          {exerciseNames.length > 0 ? (
            exerciseNames.map((exerciseName) => {
              ...
              return (
                <View key={exerciseName} className="bg-surface rounded-xl p-4 mb-3">
                  <View className="flex-row justify-between items-center mb-3">
                    ...
                    {setsForExercise.some(s => s.is_pr) && (
                      <View className="bg-yellow-500/20 px-3 py-1 rounded-full">
                        <Text className="text-yellow-400 font-bold text-xs">PR 🏆</Text>
                      </View>
                    )}
                  </View>
                  ...
                  {setsForExercise.map((set: ExerciseSet) => (
                    <View
                      key={set.id}
                      className="flex-row justify-between items-center py-2 border-t border-gray-700"
                    >
